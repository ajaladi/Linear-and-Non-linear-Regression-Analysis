...
## Prepending ones utility function
def prepend_ones(data):
	return numpy.append(numpy.ones((data.shape[0], 1)), data, axis=1)
...
def linreg_closed_form(train_X, train_Y):
	...
	X = prepend_ones(train_X)
	Theta = numpy.dot(numpy.linalg.inv(numpy.dot(X.T, X)), numpy.dot(X.T, train_Y))
	return Theta
...
def loss(Theta, train_X, train_Y):
	...
	delta = pred - train_Y
	rv = numpy.sum(delta ** 2) / (2 * X.shape[0])
	return rv
...
def linreg_grad_desc(initial_Theta, train_X, train_Y, alpha=0.05, num_iters=500, print_iters=True):
	...
	for k in range(1,num_iters+1):
		...
		gradient = (numpy.dot(numpy.dot(X.T, X), cur_Theta) - numpy.dot(X.T, train_Y)) / (X.shape[0])
...
def random_fourier_features(train_X, train_Y, num_fourier_features=100, alpha=0.05, num_iters=500, print_iters=False):
	...
	# TODO: sample inner-products
	mean = numpy.zeros(train_X.shape[1])
	cov = (2 / train_X.shape[1]) * numpy.eye(train_X.shape[1])
	Omega = numpy.transpose(numpy.random.multivariate_normal(mean, cov, size=num_fourier_features))
	# # TODO: sample translations
	B = numpy.random.uniform(0, 2*numpy.pi, size=(1, num_fourier_features))
...
def linreg(data_X, data_Y):
	...
	sq_loss = loss(Theta, data_X, data_Y)
	print(f'Loss: {sq_loss}')

	vis_linreg_model(data_X, data_Y, Theta)	

def linreg_fun(file):
	data_X, data_Y = load_data(file)
	linreg(data_X, data_Y)

def linreg_col_dup(file):
	data_X, data_Y = load_data(file)
	data_X = numpy.insert(data_X, 1, data_X[:,0], axis=1)
	linreg(data_X, data_Y)

def linreg_row_dup(file):
	data_X, data_Y = load_data(file)
	data_X = numpy.insert(data_X, 1, data_X[0], axis=0)
	data_Y = numpy.insert(data_Y, 1, data_Y[0], axis=0)
	linreg(data_X, data_Y)

## Gradient descent function
def grad_desc(data_X, data_Y, alpha, num_iters, print_iters):
	zero_vector = numpy.zeros((data_X.shape[1] + 1, 1))
	history = linreg_grad_desc(zero_vector, data_X, data_Y, alpha=alpha, num_iters=num_iters, print_iters=print_iters)
	
	disp_hist = list(map(lambda x: (x[0].tolist(), x[1]), history))
	for i, e in enumerate(disp_hist, start=1):
		print(f'Iteration {i}')
		print(f'{e}\n')

	print(f'Theta: {history[-1][0].tolist()}')
	print(f'Loss: {history[-1][1]}')

	vis_linreg_model(data_X, data_Y, history[-1][0])

def grad_desc_fun(file, alpha=0.05, num_iters=500, print_iters=False):
	data_X, data_Y = load_data(file)
	grad_desc(data_X, data_Y, alpha, num_iters, print_iters)

def grad_desc_col_dup(file, alpha=0.05, num_iters=500, print_iters=False):
	data_X, data_Y = load_data(file)
	data_X = numpy.insert(data_X, 1, data_X[:,0], axis=1)
	grad_desc(data_X, data_Y, alpha, num_iters, print_iters)

def grad_desc_row_dup(file, alpha=0.05, num_iters=500, print_iters=False):
	data_X, data_Y = load_data(file)
	data_X = numpy.insert(data_X, 1, data_X[0], axis=0)
	data_Y = numpy.insert(data_Y, 1, data_Y[0], axis=0)
	grad_desc(data_X, data_Y, alpha, num_iters, print_iters)

## Random fourier features function
def rff(file, num_fourier_features=100, num_iters=500, print_iters=False):
	data_X, data_Y = load_data(file)

	T, O, B = random_fourier_features(data_X, data_Y, num_fourier_features=num_fourier_features, num_iters=num_iters, print_iters=print_iters)

	vis_rff_model(data_X, data_Y, T, O, B)

if __name__ == '__main__':

	linreg_fun('1D-no-noise-lin.txt')
	linreg_fun('2D-noisy-lin.txt')

	linreg_col_dup('1D-no-noise-lin.txt')
	linreg_col_dup('2D-noisy-lin.txt')

	linreg_row_dup('1D-no-noise-lin.txt')
	linreg_row_dup('2D-noisy-lin.txt')

	grad_desc_fun('1D-no-noise-lin.txt')
	grad_desc_fun('2D-noisy-lin.txt')

	grad_desc_col_dup('1D-no-noise-lin.txt')
	grad_desc_col_dup('2D-noisy-lin.txt')

	grad_desc_row_dup('1D-no-noise-lin.txt')
	grad_desc_row_dup('2D-noisy-lin.txt')
	
	rff('1D-exp-samp.txt')
	rff('1D-exp-uni.txt')
	rff('1D-quad-uni.txt')
	rff('1D-quad-uni-noise.txt')
